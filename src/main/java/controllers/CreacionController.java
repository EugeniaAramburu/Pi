...
public class CreacionController {
	...
	public static List<Indicador> indicadoresCreadosPorSesion = new LinkedList<>();
	...
	public ModelAndView indicadorCreado(Request req, Response res) throws IOException, ParseException{
		...
		if(nombres.contains(nombreIndicador)){
			...
			CreacionController.indicadoresCreadosPorSesion.stream().forEach(i -> nombres.remove(i.getNombreIndicador()));
			CreacionController.indicadoresCreadosPorSesion.stream().forEach(i -> nombres.add(i.getNombreIndicador()));
