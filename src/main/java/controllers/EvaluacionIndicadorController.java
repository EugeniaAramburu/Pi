...
public class EvaluacionIndicadorController {
	...
	public static int tamanoOficial;
	...
	public ModelAndView seleccionarIndicador(Request req, Response res) throws IOException{
		...
		final List<Indicador> indicadores = em.listaIndicadoresPorId(LogInController.idUsuario);
		if(indicadores.size()==tamanoOficial){
			CreacionController.indicadoresCreadosPorSesion.stream().forEach(i -> indicadores.add(i));	
		}
	...
	public ModelAndView ingresarPeriodo(Request req, Response res) throws IOException, ParseException{
		...
		if(entitymodel.listaIndicadoresPorId(LogInController.idUsuario).size()!=tamanoOficial){
			indicador = (entitymodel.getIndicador(indicadorBuscado));	
		}else{
			List<Indicador> indi=entitymodel.listaIndicadoresPorId(LogInController.idUsuario);
			CreacionController.indicadoresCreadosPorSesion.forEach(i->indi.add(i));
			indi.stream().forEach(i->nombres.add(i.getNombreIndicador()));
			int i = nombres.indexOf(indicadorBuscado);
			indicador = indi.get(i);
		}
